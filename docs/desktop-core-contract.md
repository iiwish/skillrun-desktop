...
- `inspect` 只用于展示 catalog 和 item summary；`catalog-path-or-url` 可以是本地 JSON 路径，也可以是 Core 支持的 `http(s)` catalog URL。
...
- 远程 catalog URL 在 Desktop 中是只读入口：允许 inspect / status / install plan，不允许 apply。
- `install apply` 必须由用户确认后触发，并且只从本地 catalog path 调用。
