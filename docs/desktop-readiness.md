...
**日期**：2026-06-01
**适用范围**：`skillrun-desktop` tray-first alpha 的产品边界、Core 合同和验证路径

## 一句话判断

Desktop 已经进入 alpha 实现阶段，并且仍必须把自己定位成 **SkillRun Core 的消费者控制台**：只调用稳定 CLI JSON / Router 合同，不读取 `.skillrun/` 内部目录，不解析 Manifest YAML，不直接执行 action，不发明自己的 import、mount、run history 或 trust 语义。
...
- Team Catalog inspect / status / install plan / guarded install apply。
- Router status / dry-run route diagnostics，包括 routable / blocked、warning / error issue 和 recovery suggestion。

Desktop 仓库也已经具备 CLI runner、Core JSON parser fixtures、Team Library alpha UI、real Core smoke、hero Desktop smoke 和 Router diagnostics 手测 SOP。当前重点不再是“能否开始实现”，而是发布前持续验证真实 Core 链路、补齐手测证据，并保持 Desktop/Core 边界不漂移。
...
1. 发布前持续运行 `npm run smoke:real-core` 和 `npm run smoke:hero-desktop`，确保真实 Core 的 import、enable、exposure、Router diagnostics、Team Library install 路径没有退化。
2. 用 `router-diagnostics-manual-test.md` 做一轮有证据的手测，覆盖 no routes、routable、blocked、duplicate tool 和 Desktop refresh。
3. 收敛 Desktop alpha release gate：real Core smoke 必须先于 Tauri artifact build 成功；失败时不生成可试用 release。
